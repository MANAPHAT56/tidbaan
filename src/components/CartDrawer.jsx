...
export default function CartDrawer({
  ...
  // ── โหลด Turnstile script ครั้งเดียว ──────────────────────
  useEffect(() => {
  ...
  // ── render/destroy Turnstile widget ตาม step ──────────────
  useEffect(() => {
    if (step !== "cart" && step !== "form") {
      // ออกจาก step ที่ต้องใช้ → destroy widget + reset token
  ...
  async function handleOrder() {
    // ── ตรวจ Turnstile token ก่อนเสมอ ──────────────────────
    if (!turnstileToken) {
      setGeoError(
        lang === "th"
          ? "กรุณารอการยืนยันตัวตน (Turnstile) สักครู่..."
          : "Please wait for security verification..."
      );
      // ลอง execute widget อีกรอบ
      if (window.turnstile && turnstileWidgetId.current != null) {
      ...
      async (pos) => {
        try {
          const payload = {
            ...
            turnstileToken, // ← ส่ง token ไปด้วย
          };
          ...
          if (res.ok && data.ok) {
            ...
            // token ถูกใช้แล้ว → reset เพื่อให้ widget ออก token ใหม่
            setTurnstileToken(null);
            if (window.turnstile && turnstileWidgetId.current != null) {
  ...
  const showTurnstileWaiting =
    (step === "cart" || step === "form") && !turnstileReady;
        ...
        {step === "success" && (
          <div style={{ textAlign: "center", padding: "60px 0", flex: 1 }}>
            ...
            >
              {isTakeaway
                ? lang === "th"
                  ? "เราจะโทรยืนยันนัดรับของค่ะ"
                  : "We'll call to confirm delivery."
                : lang === "th"
                ? "รอรับอาหารได้เลยค่ะ"
            ...
            <button
              ...
              style={{
                ...btn("var(--navBg)", "var(--goldBright)", false),
                marginTop: 28,
                width: "auto",
                padding: "14px 36px",
              }}
          ...
          <div style={{ textAlign: "center", padding: "60px 0", flex: 1 }}>
            ...
            <h4
              style={{
                fontWeight: 800,
                fontSize: fs + 6,
                color: failReason === "expired" ? "var(--amber)" : "#ef4444",
                fontFamily: "'Mitr', sans-serif",
                marginBottom: 8,
              }}
            >
          ...
          >
            ...
            <div style={{ textAlign: "center" }}>
              ...
              >
                🍳{" "}
                {lang === "th"
                  ? "อาหารใช้เวลาประมาณ 8–15 นาที"
                  : "Estimated prep time: 8–15 min"}
              </div>
            </div>
            ...
            >
              ...
              >
                {lang === "th"
                  ? "หน้านี้จะอัปเดตอัตโนมัติ • QR หมดอายุใน 10 นาที"
                  : "Auto-updates • QR expires in 10 min"}
              </span>
            </div>
          ...
          >
            ...
            >
              ...
              {cart.map((item) => (
                ...
                >
                  <span>
                    {lang === "th" ? item.name : item.nameEn} x{item.qty}
                  </span>
            ...
            <div>
              {label(
                lang === "th"
                  ? "🏠 ที่อยู่จัดส่ง เช่น ตึกอะไร (ไม่บังคับ)"
                  : "🏠 Delivery Address (optional)"
              )}
              <textarea
                placeholder={lang === "th" ? "ตึก A2..." : "House no., street..."}
                value={address}
              ...
              >
                📍{" "}
                {lang === "th"
                  ? "หากไม่กรอก ระบบจะใช้ตำแหน่ง GPS ของคุณ"
                  : "If blank, GPS location will be used"}
              </div>
            </div>
            ...
            {/* Turnstile waiting indicator — form */}
            {showTurnstileWaiting && (
              <div
                style={{
                  fontSize: fs - 3,
                  color: "var(--text3)",
              ...
              >
                🔒 {lang === "th" ? "กำลังตรวจสอบความปลอดภัย..." : "Security check in progress..."}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: "auto" }}>
              ...
              <button
                ...
                disabled={checking || !turnstileReady}
                style={btn("var(--navBg)", "var(--goldBright)", checking || !turnstileReady)}
              >
                {checking
                  ...
                  : (lang === "th"
                      ? `ยืนยัน ฿${Number(total).toLocaleString()}`
                      : `Confirm ฿${Number(total).toLocaleString()}`) + " →"}
              </button>
            </div>
          </div>
        )}
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                >
                  ...
                  {/* Turnstile waiting indicator — cart */}
                  {showTurnstileWaiting && (
                    <div
                      style={{
                        fontSize: fs - 3,
                  ...
                  {isTakeaway ? (
                    <button
                      ...
                      style={btn("var(--navBg)", "var(--goldBright)", false)}
                    >
                    ...
                    >
                      {checking
                        ...
                        ? lang === "th"
                          ...
                          : "🔒 Verifying..."
                        : (lang === "th"
                            ? `สั่งอาหาร ฿${Number(total).toLocaleString()}`
                  ...
                  >
                    📍{" "}
                    {lang === "th"
                      ? "ต้องอยู่ในบริเวณร้านเพื่อสั่งอาหาร"
                      : "Must be at the restaurant to order"}
                  </p>
                </div>
              </>
            )}
